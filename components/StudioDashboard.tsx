...
import { LayoutDashboard, Users, FileText, Download, Clock, CheckCircle2, Circle, ArrowRight, Loader2 } from 'lucide-react';
...
const StudioDashboard: React.FC<Props> = ({ onExit }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans" dir="rtl">
      {/* Top Navbar */}
      <header className="bg-slate-900 text-white p-4 shadow-lg z-10 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-cyan-500 rounded-lg flex items-center justify-center">
            <LayoutDashboard className="w-6 h-6 text-white" />
          </div>
          <div>
            <h1 className="font-black text-xl tracking-tight">סטודיו אדוה קורן</h1>
            <p className="text-cyan-400 text-xs font-bold uppercase tracking-widest">מערכת ניהול איפיונים</p>
          </div>
        </div>
        <button onClick={onExit} className="text-slate-400 hover:text-white flex items-center gap-2 text-sm font-bold bg-slate-800 px-4 py-2 rounded-lg transition-colors">
          חזרה לאתר <ArrowRight className="w-4 h-4" />
        </button>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
            ...
            <div className="flex gap-2 mb-4 bg-slate-100 p-1 rounded-lg">
              {(['all', 'new', 'in_progress', 'completed'] as const).map((f) => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
                  className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${filter === f ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
        ...
        {/* Main Content Area */}
        <div className="flex-1 bg-slate-50 overflow-y-auto relative h-[calc(100vh-72px)]">
          {selectedSub ? (
            <div className="max-w-4xl mx-auto p-8 pb-32">
              {/* Header Actions */}
              <div className="flex justify-between items-center mb-8 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                <div>
                  <h2 className="text-3xl font-black text-slate-900 mb-2">{selectedSub.businessName}</h2>
                  <div className="flex gap-4">
                    <select 
                      value={selectedSub.status}
                      onChange={(e) => updateStatus(selectedSub.id, e.target.value as SubmissionStatus)}
                      className="text-sm font-bold bg-slate-100 border-none rounded-lg px-3 py-1.5 focus:ring-2 focus:ring-cyan-500 cursor-pointer"
                    >
                      <option value="new">סטטוס: חדש</option>
                      <option value="in_progress">סטטוס: בטיפול</option>
                      <option value="completed">סטטוס: סוים</option>
                    </select>
                    <button onClick={() => deleteSubmission(selectedSub.id)} className="text-sm font-bold text-red-500 hover:text-red-700 transition-colors">
                      מחק שאלון
                    </button>
                  </div>
                </div>
                ...
                <button 
                  ...
                  className="flex items-center gap-3 bg-slate-900 text-white px-6 py-3 rounded-xl font-bold hover:bg-black transition-all shadow-lg shadow-slate-200 active:scale-95 disabled:opacity-70"
                >
              ...
              <div ref={printRef} className="bg-white rounded-[2rem] p-12 shadow-xl border border-slate-100 relative">
                <div className="absolute top-0 left-0 w-full h-3 bg-gradient-to-r from-cyan-400 to-blue-500 rounded-t-[2rem]"></div>
                
                <div className="flex flex-col items-center text-center border-b-2 border-slate-100 pb-8 mb-10 mt-4">
                  <div className="text-4xl font-black text-slate-900 mb-2">אדוה קורן</div>
                  <div className="text-cyan-500 font-bold tracking-widest uppercase text-sm">סיכום שאלון איפיון מותג</div>
                </div>
...
const Section: React.FC<{title: string, children: React.ReactNode}> = ({title, children}) => (
  <div>
    <h3 className="text-lg font-black text-slate-800 border-b-2 border-slate-100 pb-2 mb-6 inline-block pr-2">{title}</h3>
...
const GridItem: React.FC<{label: string, value: string, full?: boolean, textClass?: string}> = ({label, value, full, textClass}) => (
  <div className={`flex flex-col gap-2 ${full ? 'col-span-1 md:col-span-2' : ''}`}>
    <span className="text-[11px] font-black text-cyan-500 uppercase tracking-widest">{label}</span>
    <p className={`font-bold text-slate-900 leading-relaxed text-lg ${textClass || ''}`}>
