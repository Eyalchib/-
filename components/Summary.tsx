...
import { QuestionnaireData, Submission } from '../types';
...
const Summary: React.FC<Props> = ({ data }) => {
  ...
  // Save to "Database" (localStorage mock) on mount
  useEffect(() => {
    try {
      const existingStr = localStorage.getItem('adva_submissions');
      const existing: Submission[] = existingStr ? JSON.parse(existingStr) : [];
      
      // Check if we already saved this session (basic check by business name)
      // In a real app we'd have a session ID.
      const alreadySaved = existing.some(sub => sub.businessName === data.businessName && (Date.now() - new Date(sub.submittedAt).getTime() < 60000));
      
      if (!alreadySaved && data.businessName) {
        ...
        localStorage.setItem('adva_submissions', JSON.stringify([newSubmission, ...existing]));
      }
    } catch (e) {
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      try {
        await navigator.share({
          files: [file],
          title: `שאלון איפיון - ${data.businessName}`,
          text: `שלום אדוה, מצורף שאלון האיפיון שמילאתי באתר עבור המותג: ${data.businessName}`,
        });
        return;
      } catch (err) {
        console.log('Share was cancelled or not supported for this file type.');
...
שלום אדוה, מילאתי שאלון איפיון באתר.
*** הקובץ ירד הרגע למחשב שלי, אני מצרף/ת אותו למייל זה ***
...
${data.businessName || 'לקוח השאלון'}
    ...
    setTimeout(() => {
      ...
      alert('מגבלות דפדפן לא מאפשרות צירוף אוטומטי. הקובץ ירד הרגע למחשב - אנא צרף אותו ידנית לחלון המייל שנפתח.');
    }, 500);
  };
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="text-center">
        ...
        <h3 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">האיפיון נשמר במערכת!</h3>
      ...
      <div className="flex flex-col gap-4 max-w-lg mx-auto w-full pt-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          >
            {isGenerating ? <Loader2 className="w-5 h-5 animate-spin" /> : <Mail className="w-5 h-5 group-hover:translate-x-1 transition-transform" />}
            <span>שלח במייל</span>
          </button>
